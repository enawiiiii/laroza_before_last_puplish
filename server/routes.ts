...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/products/:id/inventory", async (req, res) => {
    try {
      const inventory = await storage.getAllProductInventory(req.params.id);
      res.json(inventory);
    } catch (error) {
  ...
  app.post("/api/sales", async (req, res) => {
    try {
      ...
      for (const item of itemsData) {
        const inventory = await storage.getProductInventory(item.productId, updatedSaleData.storeType);
